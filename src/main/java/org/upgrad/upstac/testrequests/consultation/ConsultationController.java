...
public class  ConsultationController {
    ...
    public List<TestRequest> getForDoctor()  {
        ...
        List<TestRequest> list = new ArrayList<TestRequest>();

        try {
            currentUser = userLoggedInService.getLoggedInUser();
            if (currentUser.doesRoleIsDoctor()) {
                list = testRequestQueryService.findByDoctor(currentUser);
                log.info("Test Requests: " + list );
            }
        }
        catch (Exception e ){
            log.error("Error fetching Test Requests" + e.getMessage());
